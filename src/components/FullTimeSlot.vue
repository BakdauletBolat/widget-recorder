<template>
    <v-chip-group selected-class="text-deep-purple-accent-4">
        <div @click="() => { appStore.selected_date = slot.date; appStore.selected_employee_id = employee.id }"
            v-for="slot in slots" :key="slot.date.toString()">
            <v-chip label :disabled="slot.isReserved" :value="slot.date">
